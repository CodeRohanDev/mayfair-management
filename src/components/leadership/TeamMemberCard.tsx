...
export default function TeamMemberCard({ member, disableClick = false }: TeamMemberCardProps) {
  ...
  const CardContent = () => (
    <div className={`bg-white shadow-sm overflow-hidden ${!disableClick ? 'hover:shadow-lg cursor-pointer' : ''} transition-all duration-300 flex flex-col h-full`}>
      {/* Photo - only show if valid image exists */}
      {hasValidImage && (
        <div className="aspect-[3/4] bg-gray-200 relative overflow-hidden group">
          {/* Teal hover overlay - only on image and only if clickable */}
          {!disableClick && (
          ...
          <img
            src={member.image}
            alt={member.name}
            className={`w-full h-full object-cover ${!disableClick ? 'group-hover:scale-105' : ''} transition-transform duration-300`}
            onError={(e) => {
